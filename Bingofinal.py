...
class Spieler:
    def __init__(self, name, pipe_name, pos, size=(50, 20),words=[]):
        """
    ...
    def clear_layout(self):
          while self.winLayout.children():
            item = self.winLayout.children()[0]
            if item:
                item.setParent(None)
    ...
    def create_bingo_card(self, felder_Anzahl, words):
        ...
        try:
            ...
            # Bingo- , Spiel Beenden- und neue runde-Buttons hinzufügen
            ...
            self.neue_runde_button = TTkButton(text="Neue Runde", parent=self.bingoFenster,border=True)
            self.neue_runde_button.clicked.connect(self.neue_runde)
            self.winLayout.addWidget(self.neue_runde_button,self.felder_Anzahl + 2,0,1,felder_Anzahl )
    ...
    def show_new_round_button(self):
        #erstellen eine neue runde funktion
        self.neue_runde_button = TTkButton(text="Neue Runde",parent=self.bingoFenster, border=True)
        self.neue_runde_button.clicked.connect(Spieler.neue_runde)
        self.winLayout.addWidget(self.neue_runde_button,self.felder_Anzahl + 2,0,1,self.felder_Anzahl )
        neuerundeFenster = ttk.TTkWindow( parent=self.root,  title="Neue Runde", size=(30, 5))
    
    # Positioniere das Fenster in der oberen rechten Ecke des Terminals
        terminal_width = self.root.width()
        neuerundeFenster.move(terminal_width - 50, 0)
        neuerundeLayout = ttk.TTkLayout()
        neuerundeFenster.setLayout(neuerundeLayout)
        neuerundeLabel = ttk.TTkLabel(text=f"{self.name} neue runde")
        neuerundeLayout.addWidget(neuerundeLabel)
        neuerundeFenster.show()
    ...
    def neue_runde(self):
        ...
        self.clear_layout()  # löscht layout
        self.has_won = False  # stellt gewinner status auf falsch
        self.logger.info(f"{datetime.now().strftime('%Y-%m-%d-%H-%M-%S')} Neue Runde")
        self.create_bingo_card(self.felder_Anzahl, self.words.copy())  # bildet eine neue bingo karte
        
        
    def clear_layout(self):
      for item in reversed(range(self.winLayout.count())):
        widget = self.winLayout.itemAt(item).widget()
    ...
    # um das spiel nochmal zu starten
     
     self.clear_layout()  
     self.has_won = False
     self.create_bingo_card(self.felder_Anzahl, self.words)
...
def client_process(name, pipe_name, pos, size, felder_Anzahl, words):
    ...
    try:
        spieler = Spieler(name, pipe_name, pos, size,words)
